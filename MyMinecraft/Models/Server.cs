...
public class Server {
    ...
    public Server(World world, int renderDistance) : this() {
        this.world = world;
    ...
    private Chunk? GenChunk(Vector3i chunkPos, bool usedForGen) {
    ...
    private void TryPlaceBlock(PlaceBlockPacket? packet) {
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            if (y<Chunk.HEIGHT && world.GetChunk(chunkPos, out Chunk? chunk)) {
                BlockType block = chunk.GetBlockTypeAt(chunkBlockPos);

                if (prevValSet && (prevChunkPos == chunkPos || !world.IsLoadedChunk(prevChunkPos)))
                    prevBlock = chunk.GetBlockTypeAt(prevBlockPos);
                ...
                if (block != BlockType.AIR && prevValSet && prevBlock == BlockType.AIR) {
                    ...
                    try {
                        ...
                        AABB playerAABB = Camera.GetPlayerHitbox(packet.position);
                        AABB blockAABB = new AABB(prevBlockPos, prevBlockPos + Vector3.One);

                        if (playerAABB.Intersects(blockAABB)) {
                            Console.WriteLine("ERROR: [ Can't place block inside player]");
                            return;
                        }
    ...
    private void MeshChunk(ref Chunk chunk) {
        ...
        for (var x = 0; x<Chunk.SIZE; ++x) {
            for (var z = 0; z<Chunk.SIZE; ++z) {
                for (var y = 0; y<Chunk.HEIGHT; ++y) {
                    if (chunk.GetBlockTypeAt(x, y, z) != BlockType.AIR) {
                        uint addedFaces = 0; // Reset for each block
                        Vector3 blockPos = new(x, y, z);
                        var currBlock = chunk.GetBlockAt(x,y,z);
                        // Left face
                        ...
                        if ((neighbourBlock==BlockType.AIR && x==0) || (x!=0 && chunk.GetBlockTypeAt(x-1, y, z) == BlockType.AIR)) {
                        ...
                        // Right face
                        if ((neighbourBlock==BlockType.AIR && x == Chunk.SIZE - 1) || (x != Chunk.SIZE-1 && chunk.GetBlockTypeAt(x + 1, y, z) == BlockType.AIR)) {
                        ...
                        // Bottom face
                        if (y == 0 || chunk.GetBlockTypeAt(x, y - 1, z) == BlockType.AIR) {
                        ...
                        // Top face
                        if (y == Chunk.HEIGHT - 1 || chunk.GetBlockTypeAt(x, y + 1 , z)== BlockType.AIR) {
                        ...
                        neighbourBlock = world.GetBlockAt(neighbourChunkPos, neighbourBlockPos);
                        // Back face
                        if ((neighbourBlock == BlockType.AIR && z == 0)|| (z != 0 && chunk.GetBlockTypeAt(x, y, z-1) == BlockType.AIR)) {
                        ...
                        neighbourBlock = world.GetBlockAt(neighbourChunkPos, neighbourBlockPos);
                        // Front face
                        if ((neighbourBlock == BlockType.AIR && z == Chunk.SIZE - 1) || (z != Chunk.SIZE-1 && chunk.GetBlockTypeAt(x, y, z + 1) == BlockType.AIR)) {
    ...
    public void AddChunkToMesh(Chunk chunk) {
        chunksToMesh.Enqueue(chunk);
    }
